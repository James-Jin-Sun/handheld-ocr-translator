...
/// (wireless_mvp/firmware/esp32_camera) for Android / desktop / tests.
///
/// Browsers decode `multipart/x-mixed-replace` inside `<img>`; Dart IO does
/// not, so this parses the firmware's `--frame` parts (Content-Length + JPEG
/// bytes) and paints each frame with [Image.memory].
library;
...
class _MjpegViewState extends State<MjpegView> {
  ...
  void _start() {
    _stop();
    // Widget tests still compile this IO viewer; skip the socket so
    // `flutter test` doesn't wait on mDNS / a real ESP32.
...
/// Pulls JPEG payloads out of the ESP32 firmware's multipart body:
/// `--frame\r\nContent-Type: image/jpeg\r\nContent-Length: N\r\n\r\n` + N bytes.
class _MjpegStreamParser {
  static const _maxBufferBytes = 2 * 1024 * 1024;
  static const _boundary = [0x2d, 0x2d, 0x66, 0x72, 0x61, 0x6d, 0x65]; // --frame
  ...
  Uint8List? takeFrame() {
    while (true) {
      final boundaryAt = _indexOf(_boundary);
      if (boundaryAt == null) {
      ...
      if (boundaryAt > 0) {
        _buffer.removeRange(0, boundaryAt);
      }
      ...
      if (match == null) {
        _buffer.removeRange(0, _boundary.length);
        continue;
      }
