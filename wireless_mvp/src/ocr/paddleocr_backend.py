...
try:
    # Must be imported before `paddle` -- on Windows, paddle's bundled
    # OpenMP/MKL DLLs load first and shadow a dependency of torch's
    # shm.dll, so importing torch afterwards fails with WinError 127.
    # torch isn't installed on every target (e.g. Jetson), so ignore if missing.
...
def _cached_construct(key, factory):
    """Build (and permanently memoize) a Paddle predictor for `key`.

    PaddleX's inference backend is a process-wide singleton that can only be
    initialized once; retrying a failed construction doesn't get a second
    chance, it just fails with a confusing "PDX has already been
    initialized" error that hides the real cause. So a failed attempt is
    cached too, and re-raised as-is on every later call for the same key
    instead of attempting (and re-failing) construction again.
    """
    with _construct_lock:
        if key in _construct_errors:
            raise _construct_errors[key]
        if key in _constructed:
            return _constructed[key]
        try:
            instance = factory()
        except Exception as exc:
            _construct_errors[key] = exc
            raise
        _constructed[key] = instance
        return instance


def get_paddle_pipeline(lang, device):
    return _cached_construct(
        ("paddle_pipeline", lang, device),
        lambda: PaddleOCR(
            lang=lang,
            device=device,
...
def get_text_detector(device):
    return _cached_construct(("text_detector", device), lambda: TextDetection(device=device))


def get_text_recognizer(device):
    return _cached_construct(("text_recognizer", device), lambda: TextRecognition(device=device))
